...
//                     filterOptions.find(
//                       (filter) => filter.value === machine.status
//                     )?.color || "bg-gray-300"
//                   }`}
//                 >
//                   {machine.status}
      ...
      <div className="space-y-4 mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          {filterOptions.map((filter) => (
            <Badge
              key={filter.value}
              className={`cursor-pointer ${filter.color} text-white hover:bg-opacity-80`}
            >
              {filter.label}
            </Badge>
          ))}
        ...
        {machines.map((machine) => (
          ...
          >
            <CardContent className="p-4 h-full flex flex-col">
              ...
              <h3 className="mt-4">{machine.name}</h3>
            </CardContent>
          </Card>
        ))}
      </div>

      {/* Modal for Machine Profile */}
      {selectedMachine && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="relative bg-white rounded-lg shadow-xl w-11/12 max-w-3xl p-6 overflow-y-auto max-h-[90vh]">
            ...
            {/* Machine Profile Content */}
            <MachineProfile machine={selectedMachine} />
          </div>
        </div>
      )}
    </div>
